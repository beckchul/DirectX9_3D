...
CMonsterHpBar::CMonsterHpBar(LPDIRECT3DDEVICE9 pDevice)
...
CMonsterHpBar::~CMonsterHpBar(void)
{
	Release();
}
...
HRESULT CMonsterHpBar::Initialize(void)
{
	...
	m_fX = float(WINCX / 2);
	m_fY = float(WINCY / 8);
	m_fSizeX = float(WINCX) * 0.6f;
	m_fSizeY = float(WINCY) * 0.05f;
...
void CMonsterHpBar::Update(void)
{
...
HRESULT CMonsterHpBar::AddComponent(void)
{
	...
	//Texture
	pComponent = m_pResourceMgr->CloneResource(RESOURCE_STAGE, L"MonsterHpBar");
...
void CMonsterHpBar::SettingHpBar(void)
{
	int iHp = (int)*m_pUIObserver->GetMonsterHp();
	int iHpMax = (int)*m_pUIObserver->GetMonsterHpMax();
...
CMonsterHpBar * CMonsterHpBar::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CMonsterHpBar*	pGameObject = new CMonsterHpBar(pDevice);
