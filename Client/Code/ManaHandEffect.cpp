...
CManaHandEffect::CManaHandEffect(LPDIRECT3DDEVICE9 pDevice)
...
CManaHandEffect::~CManaHandEffect(void)
{
	Release();
}

HRESULT CManaHandEffect::Initialize(void)
{
	AddComponent();
...
void CManaHandEffect::Update(void)
{
...
void CManaHandEffect::Render(void)
{
	if (!m_bRender)
		return;
	...
	((Engine::CRcTex*)m_pBuffer)->SetEffectUV(5.f, 7.f, static_cast<float>(iPosX), static_cast<float>(iPosY));
...
void CManaHandEffect::Release(void)
{
}

HRESULT CManaHandEffect::AddComponent(void)
{
	...
	//Texture
	pComponent = Engine::Get_ResourceMgr()->CloneResource(RESOURCE_STAGE, L"ManaHand");
...
HRESULT CManaHandEffect::SetConstantTable(LPD3DXEFFECT pEffect)
{
	...
	D3DXVECTOR3 vPos = m_vEffectPos + D3DXVECTOR3(0.f, 0.3f, 0.f);
...
void CManaHandEffect::State(void)
{
	if (!m_bRender)
		return;

	m_fCount += Engine::Get_TimeMgr()->GetTime() * 15.f;
...
CManaHandEffect* CManaHandEffect::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CManaHandEffect*	pGameObject = new CManaHandEffect(pDevice);
...
void CManaHandEffect::SetEffectPos(D3DXVECTOR3 _vPos)
{
	m_vEffectPos = _vPos;
}

void CManaHandEffect::SetRender(bool _bRender)
{
	m_bRender = _bRender;
}
