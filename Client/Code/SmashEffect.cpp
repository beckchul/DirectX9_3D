...
CSmashEffect::CSmashEffect(LPDIRECT3DDEVICE9 pDevice)
...
CSmashEffect::~CSmashEffect(void)
{
	Release();
}

HRESULT CSmashEffect::Initialize(void)
{
	AddComponent();
...
void CSmashEffect::Release(void)
{
}

HRESULT CSmashEffect::AddComponent(void)
{
	...
	//Texture
	pComponent = Engine::Get_ResourceMgr()->CloneResource(RESOURCE_STAGE, L"SmashEffect");
...
HRESULT CSmashEffect::SetConstantTable(LPD3DXEFFECT pEffect)
{
...
void CSmashEffect::State(void)
{
	if (!m_bRender)
		return;

	m_fCount += Engine::Get_TimeMgr()->GetTime() * 90.f;
...
CSmashEffect * CSmashEffect::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CSmashEffect*	pGameObject = new CSmashEffect(pDevice);
...
void CSmashEffect::SetEffectPos(D3DXVECTOR3 _vPos)
{
	m_vEffectPos = _vPos;
}

void CSmashEffect::SetRender(bool _bRender)
{
	m_bRender = _bRender;
}
