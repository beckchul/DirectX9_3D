...
PS_OUT PS_MAIN(PS_IN In)
{
		...
		&& In.vWorldPos.z < g_vBrushPos.z + g_fBrushSize)
	{
		...
		vColor = tex2D(BrushSampler, vNewUV);
	}


	Out.vColor = Out.vColor * (In.vShade * 0.4f) + In.vSpecular * 0.2f + vColor;

	if (g_fBlack == 0.f)
	{
		Out.vColor.r *= g_fColor;
		Out.vColor.g *= g_fColor;
		Out.vColor.b *= g_fColor;
	}
